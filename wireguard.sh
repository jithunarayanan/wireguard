...
if [ ! -d /root/wg-clients ]; then
	mkdir -p /root/wg-clients
fi
#Please check config file options for Line No: 566
...
if [[ $(uname -r | cut -d "." -f 1) -eq 2 ]]; then
	echo "Your system is running an old kernel, which is  not compatible with this installer."
	exit
fi
...
EOF
	...
	case "$option" in
		1)
			...
			qrencode -t UTF8 < ~/"$client.conf" >> ~/wg-clients/"$client"-qr.txt
			echo
			qrencode -t UTF8 < ~/"$client.conf"
			echo -e '\xE2\x86\x91 That is a QR code containing your client configuration.'
			echo
			#echo "$client added. Configuration available in:" ~/"$client.conf"
			echo "$client added. Configuration available in:" ~/wg-clients"$client.conf"
			mv ~/"$client.conf" ~/wg-clients/
			exit
		;;
		2)
